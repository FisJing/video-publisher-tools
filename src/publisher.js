...
class WeChatVideoPublisher {
    ...
    adb(cmd) {
        ...
        // 如果有设备ID，添加 -s 参数
        if (deviceId) {
            fullCmd = `-s ${deviceId} ${cmd}`;
        }

        // 分身微信需要添加 --user 参数
        if (cmd.includes('am start')) {
            fullCmd = fullCmd.replace('am start', `am start --user ${userId}`);
        } else if (cmd.includes('ime enable') && userId !== 0) {
            fullCmd = fullCmd.replace('ime enable', `ime enable --user ${userId}`);
        } else if (cmd.includes('ime set') && userId !== 0) {
            fullCmd = fullCmd.replace('ime set', `ime set --user ${userId}`);
        } else if (cmd.includes('am broadcast') && userId !== 0) {
            fullCmd = fullCmd.replace('am broadcast', `am broadcast --user ${userId}`);
        }
     ...
     */
    disableSystemIME() {
        if (this.config.userId === 0) return;  // 机主微信不需要

        console.log('  禁用系统输入法...');
        const userId = this.config.userId;

        // 小米/红米手机
        this.adb(`shell cmd input_method ime disable --user ${userId} com.baidu.input_mi/.ImeService`);
        this.adb(`shell cmd input_method ime disable --user ${userId} com.sohu.inputmethod.sogou.xiaomi/.SogouIME`);
        this.adb(`shell cmd input_method ime disable --user ${userId} com.iflytek.inputmethod.miui/.FlyIME`);

        // OPPO/一加手机
        this.adb(`shell cmd input_method ime disable --user ${userId} com.iflytek.inputmethod.oppo/.FlyIME`);
        this.adb(`shell cmd input_method ime disable --user ${userId} com.baidu.input_oppo/.ImeService`);

        // 华为手机
        this.adb(`shell cmd input_method ime disable --user ${userId} com.baidu.input_huawei/.ImeService`);

        console.log('  ✓ 已禁用系统输入法');
    }
    ...
    async inputText(text) {
        ...
        // 步骤1: 切换到ADBKeyboard输入法
        console.log('  1. 切换输入法...');

        // 分身微信需要先禁用系统输入法
        if (this.config.userId !== 0) {
            this.disableSystemIME();
        }
        ...
        for (let i = 0; i < parts.length; i++) {
            const partWithSpace = (i < parts.length - 1) ? parts[i] + ' ' : parts[i];
            this.adb(`shell am broadcast --user ${this.config.userId} -a ADB_INPUT_TEXT --es msg "${partWithSpace}"`);
